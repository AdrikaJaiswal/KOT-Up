// ==UserScript==
// @name         Swiggy Zomato Order Forwarder (Dual-Format Master)
// @namespace    http://tampermonkey.net/
// @version      2.8
// @description  Simultaneously extract inline brackets [X Pcs] and subscript text on both platforms.
// @author       Adrika
...
function parseZomatoCard(card) {
    ...
    // Scan all possible order line wrappers natively
    ...
    itemRows.forEach(row => {
        ...
        if (nameEl && qtyEl) {
            ...
            // CHECK TYPE 1: Inline bracket check rule (e.g. "Peas Kachori [3 Pieces]")
            const inlineBracketMatch = rawName.match(/\[(\d+)\s*pieces?\]/i);
            // CHECK TYPE 2: Subscript context check rule (e.g. "Quantity: 8 Pieces")
            ...
            // Avoid double counting row iterations
...
function scanDashboardLayout() {
    ...
    if (isSwiggy) {
        ...
        document.querySelectorAll('.css-eeodfr').forEach(card => {
            // Confirm card contains standard cooking queue token boundaries
...
if (window.location.hostname.includes('whatsapp.com')) {
    ...
    console.log("%c[Forwarder] Dual-Format evaluation locked!", "color: #00ffff; font-weight: bold;");
